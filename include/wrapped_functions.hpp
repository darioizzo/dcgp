...
template <typename T>
using f_enabler = typename std::enable_if<
    std::is_same<T, double>::value || is_gdual<T>::value, int>::type;
 ...
 *                                  N-ARITY FUNCTIONS
 *------------------------------------------------------------------------**/
template <typename T, f_enabler<T> = 0> T my_sum(const std::vector<T> &in) {
  T retval(in[0]);
...
template <typename T, f_enabler<T> = 0> T my_diff(const std::vector<T> &in) {
  T retval(in[0]);
  for (auto i = 1u; i < in.size(); ++i) {
    retval -= in[i];
  }
...
template <typename T, f_enabler<T> = 0> T my_mul(const std::vector<T> &in) {
  T retval(in[0]);
  for (auto i = 1u; i < in.size(); ++i) {
    retval *= in[i];
  }
...
template <typename T, f_enabler<T> = 0> T my_div(const std::vector<T> &in) {
  T retval(in[0]);
  for (auto i = 1u; i < in.size(); ++i) {
    retval /= in[i];
  }
...
template <typename T, f_enabler<T> = 0> T my_sig(const std::vector<T> &in) {
  T retval(in[0]);
  for (auto i = 1u; i < in.size(); ++i) {
    retval += in[i];
  }
...
template <typename T, f_enabler<T> = 0> T my_pdiv(const std::vector<T> &in) {
  if (in[0] == in[1]) {
    return T(1.);
  }
  return in[0] / in[1];
 ...
 *------------------------------------------------------------------------**/
// sine
template <typename T, f_enabler<T> = 0> T my_sin(const std::vector<T> &in) {
  return sin(in[0]);
}
...
// cosine
template <typename T, f_enabler<T> = 0> T my_cos(const std::vector<T> &in) {
  return cos(in[0]);
}
...
// logarithm
template <typename T, f_enabler<T> = 0> T my_log(const std::vector<T> &in) {
...
// exponential
template <typename T, f_enabler<T> = 0> T my_exp(const std::vector<T> &in) {
