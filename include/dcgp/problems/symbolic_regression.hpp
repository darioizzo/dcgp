...
public:
          ...
          m_multi_objective(multi_objective), m_parallel_batches(parallel_batches), m_loss_s(loss_s)
    {
        unsigned n;
        unsigned m;
        // We check the input against obvious sanity criteria. (this also sets n = input_dim and m = label_dim)
        sanity_checks(n, m);
        // with BCE loss we have one more label than we need outputs
        if (loss_s == "BCE") {
            m -= 1;
        }
