                  ...
                  std::vector<dcgp::basis_function<gdual_d>> function_set)
{
    ...
    dcgp::expression<gdual_d> ex(in, out, rows, columns, levels_back, arity, function_set, 123);
...
BOOST_AUTO_TEST_CASE(evaluation_speed)
{
    ...
    dcgp::function_set<gdual_d> function_set1({"sum","diff","mul","div"});
    ...
    dcgp::function_set<gdual_d> function_set2({"sum","mul","sig"});
