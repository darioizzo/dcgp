...
{
    BOOST_CHECK_NO_THROW(es4cgp(0u, 2u, 0., 0u));
    BOOST_CHECK_THROW(es4cgp(1u, 0u, 1e-4, 0u), std::invalid_argument);
    BOOST_CHECK_THROW(es4cgp(1u, 2u, -1e-4, 0u), std::invalid_argument);
}

BOOST_AUTO_TEST_CASE(evolve_test)
{
    // We test that evolve fails on UDPs that are not suitable.
    es4cgp uda(10u, 2u, 1e-4, 0u);
